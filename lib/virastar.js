...
(function (name, global, definition) {
  ...
  Virastar.prototype.cleanup = function (text, options) {
    ...
    if (opts.fix_suffix_spacing) {
		...
		text = text.replace(/\u0020(ام|ات|اش|ای|اید|ایم|اند)\s/g,'‌\u200C$1\u0020');
      text = text.replace(/\u0020(ها(ی)?\s+)/g, '\u200C$1'); // in case you can not read it: \s+(tar(i(n)?)?|ha(ye)?)\s+
      text = text.replace(/\u0020(تر((ی)|(ین))?\s+)/g, '\u200C$1'); // in case you can not read it: \s+(tar(i(n)?)?|ha(ye)?)\s+
	  
	  text = text.replace(/\u0020(هایی|هایم|هایت|هایش|هایمان|هایتان|هایشان)/g, '\u200C$1'); 
